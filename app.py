...
import re
from databases import data_correctly, registry_new_acc, check_token, generate_token, get_id_by_email
...
def login():
    if request.cookies.get('token'):
        ...
        if check_token(int(id), token) == 0:
            return redirect('/me')
    if request.method == "POST":
        email = request.form.get("email")
        password = request.form.get("password")
        if data_correctly(email, password) == 0:
            response = make_response(redirect("/me"))
            id = get_id_by_email(email=email)
...
def registration():
    ...
    if request.method == "POST":
        ...
        if not errors:
            token, user_id = registry_new_acc(username=name, password=password, email=email)
